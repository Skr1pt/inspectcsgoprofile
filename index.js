...
app.get('/get-user', async (req,res) =>{
    ...
    res.status(200).send(await lookupProfile(user))
})

async function lookupProfile(user){
    let freeBot = botController.getFreeBot();

    if (freeBot) return freeBot.sendGetUserRequest(user);
    else return {success:false, msg:'No bots available'};
}
