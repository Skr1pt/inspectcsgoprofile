...
class Bot extends EventEmitter {
    ...
    logIn(username, password, auth) {
        ...
        try {
            ...
            if (this.steamClient.steamID !== null) {
    ...
    bindEventHandlers() {
        this.on('error', (err)=>{
            winston.error('got shity error', err)
        })
        ...
        this.csgoClient.on('inspectItemInfo', (itemData) => {
            if (this.resolve && this.currentRequest) {
                ...
                let delay = this.settings.request_delay;
        ...
        this.csgoClient.on('playersProfile', profile=>{
            ...
            this.resolve({success:true, profile});
            this.currentRequest = false;
    ...
    sendGetUserRequest(user){
        return new Promise((resolve, reject) => {
            ...
            if (!this.ready) {
                ...
                this.csgoClient.requestPlayersProfile(user)
            }
            ...
            this.ttlTimeout = setTimeout(() => {
                ...
                this.busy = false;
                this.currentRequest = false;
                return resolve({success:false, msg:'Timeout. Please slow down'})
